...
public class ReplacedCreeperEntity extends ReplacedEntityBase {
    protected static final RawAnimation SWELL = RawAnimation.begin().thenPlay("swell");
    private boolean isSwelling;

    public ReplacedCreeperEntity(EntityType<?> type) {
        super(type);
    }
    ...
    protected <P extends GeoReplacedEntity> PlayState predicate(AnimationState<P> event) {
        if (this.isDead) {
            ...
            event.getController().setAnimation(SWELL);
        } else {
